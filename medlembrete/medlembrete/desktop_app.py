...
from src.models import (
    Agendamento,
    DosagemObrigatoriaError,
    EmailJaCadastradoError,
    HorarioInvalidoError,
    Medicamento,
    NomeObrigatorioError,
    QuantidadeNegativaError,
    ServicoNotificacao,
    SistemaCadastro,
    TipoInvalidoError,
)
...
class MedLembreteApp(ctk.CTk):
    ...
    def mostrar_home(self):
        ...
        medicamentos = sorted(
            ...
            key=lambda m: m.chave_ordenacao_proximo_horario(datetime.now().strftime("%H:%M")),
        )
        ...
        for med in medicamentos:
            ...
            def abrir_edicao(med=med):
                self.mostrar_editar_medicamento(med)

            ctk.CTkButton(
                topo, text="✏️ Editar", width=64, height=24, corner_radius=8,
                fg_color="transparent", hover_color=COR_PRIMARIA_CLARA,
                text_color=COR_PRIMARIA, font=F_PEQUENA, command=abrir_edicao,
            ).pack(side="right")

            if med.tipo:
                self.badge(topo, med.tipo, COR_PRIMARIA_CLARA, COR_PRIMARIA_HOVER).pack(side="right", padx=(0, 6))
            ...
            if not med.horarios:
                ...
                for horario in med.horarios:
                    ...
                    if agendamento.status == Agendamento.TOMADO:
                        ...
                        self.badge(
                            linha_topo, f"⏳ {horario} — pendente", COR_PENDENTE_BG, COR_PENDENTE
                        ).pack(side="left")
                        ...
                        ctk.CTkButton(
                            linha_topo, text="Marquei como tomado", height=26, corner_radius=8,
                            fg_color=COR_PRIMARIA, hover_color=COR_PRIMARIA_HOVER,
                            font=F_PEQUENA, command=marcar_tomado,
                        ).pack(side="right")

                        if agendamento.lembretes_extras_enviados > 0:
                            self.badge(
                                linha,
                                f"🔁 {agendamento.lembretes_extras_enviados}/"
                                f"{self.servico.MAXIMO_LEMBRETES_EXTRAS} lembretes extras enviados",
                                COR_ERRO_BG, COR_ERRO,
    ...
    # ------------------------------------------------------------------
    # Tela: Editar medicamento — dosagem, tipo e quantidade (PB06/PB08/PB09)
    # ------------------------------------------------------------------
    def mostrar_editar_medicamento(self, medicamento):
        self.limpar_tela()
        self.criar_cabecalho(
            f"Editar {medicamento.nome}",
            "Dosagem, tipo de uso e quantidade disponível",
            on_voltar=self.mostrar_home,
        )
        self.criar_navbar("home")
        ...
        # --- Card: Dosagem (PB06) ---
        card_dosagem = self.cartao(content)
        card_dosagem.pack(fill="x", padx=16, pady=(16, 8))

        ctk.CTkLabel(card_dosagem, text="Dosagem", font=F_SUBTITULO,
                      text_color=COR_TEXTO).pack(anchor="w", padx=16, pady=(14, 4))
        entry_dosagem = ctk.CTkEntry(card_dosagem, height=40)
        entry_dosagem.insert(0, medicamento.dosagem or "")
        entry_dosagem.pack(fill="x", padx=16)

        def salvar_dosagem():
            try:
                medicamento.configurar_dosagem(entry_dosagem.get().strip())
                self.salvar()
                messagebox.showinfo("Dosagem atualizada", "A dosagem foi atualizada com sucesso!")
                self.mostrar_editar_medicamento(medicamento)
            except DosagemObrigatoriaError as exc:
                messagebox.showerror("Dosagem obrigatória", str(exc))

        ctk.CTkButton(
            card_dosagem, text="Salvar dosagem", command=salvar_dosagem, height=36,
            fg_color=COR_PRIMARIA, hover_color=COR_PRIMARIA_HOVER,
            font=F_TEXTO_SEC, corner_radius=8,
        ).pack(fill="x", padx=16, pady=(10, 16))

        # --- Card: Tipo de uso (PB08) ---
        card_tipo = self.cartao(content)
        card_tipo.pack(fill="x", padx=16, pady=8)

        ctk.CTkLabel(card_tipo, text="Tipo de uso", font=F_SUBTITULO,
                      text_color=COR_TEXTO).pack(anchor="w", padx=16, pady=(14, 4))
        seg_tipo = ctk.CTkSegmentedButton(
            card_tipo, values=list(Medicamento.TIPOS_VALIDOS),
            selected_color=COR_PRIMARIA, selected_hover_color=COR_PRIMARIA_HOVER,
            unselected_color=COR_FUNDO, text_color=COR_TEXTO,
        )
        seg_tipo.set(medicamento.tipo or Medicamento.TIPOS_VALIDOS[0])
        seg_tipo.pack(fill="x", padx=16)

        def salvar_tipo():
            try:
                medicamento.configurar_tipo(seg_tipo.get())
                self.salvar()
                messagebox.showinfo("Tipo atualizado", "O tipo de uso foi atualizado com sucesso!")
                self.mostrar_editar_medicamento(medicamento)
            except TipoInvalidoError as exc:
                messagebox.showerror("Tipo inválido", str(exc))

        ctk.CTkButton(
            card_tipo, text="Salvar tipo de uso", command=salvar_tipo, height=36,
            fg_color=COR_PRIMARIA, hover_color=COR_PRIMARIA_HOVER,
            font=F_TEXTO_SEC, corner_radius=8,
        ).pack(fill="x", padx=16, pady=(10, 16))

        # --- Card: Quantidade disponível (PB09) ---
        card_qtd = self.cartao(content)
        card_qtd.pack(fill="x", padx=16, pady=8)

        ctk.CTkLabel(card_qtd, text="Quantidade disponível", font=F_SUBTITULO,
                      text_color=COR_TEXTO).pack(anchor="w", padx=16, pady=(14, 4))
        entry_qtd = ctk.CTkEntry(card_qtd, height=40)
        entry_qtd.insert(
            0, str(medicamento.quantidade_disponivel)
            if medicamento.quantidade_disponivel is not None else ""
        )
        entry_qtd.pack(fill="x", padx=16)

        def salvar_quantidade():
            valor_str = entry_qtd.get().strip()
            try:
            ...
            try:
                medicamento.configurar_quantidade_disponivel(valor)
                self.salvar()
                messagebox.showinfo("Quantidade atualizada", "A quantidade disponível foi atualizada!")
                self.mostrar_editar_medicamento(medicamento)
            except QuantidadeNegativaError as exc:
                messagebox.showerror("Quantidade inválida", str(exc))

        ctk.CTkButton(
            card_qtd, text="Salvar quantidade", command=salvar_quantidade, height=36,
            fg_color=COR_PRIMARIA, hover_color=COR_PRIMARIA_HOVER,
            font=F_TEXTO_SEC, corner_radius=8,
        ).pack(fill="x", padx=16, pady=(10, 16))
    ...
    def mostrar_notificacoes(self):
        ...
        ctk.CTkLabel(
            card, text="O app verifica, com base na data/hora informadas, quais "
                       "medicamentos devem gerar lembrete agora. Se o horário "
                       "passar sem confirmação, novos lembretes são enviados a "
                       "cada 10 minutos, até 3 vezes — tudo processado "
